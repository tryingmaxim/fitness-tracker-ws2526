...
public class TrainingSessionController {
	...
	public record TrainingSessionResponse(Long id, Long planId, String name, LocalDate scheduledDate,
	...
	private static TrainingSessionResponse toDto(TrainingSession s, boolean includeExecutions) {
		...
		return new TrainingSessionResponse(s.getId(), s.getPlan() != null ? s.getPlan().getId() : null, s.getName(),
				s.getScheduledDate(), execs);
	}

	private static ExecutionResponse toDto(ExerciseExecution e) {
		return new ExecutionResponse(e.getId(), e.getExercise() != null ? e.getExercise().getId() : null,
				e.getOrderIndex(), e.getNotes());
	}

	// --- Endpunkte ---

	// Liste aller Sessions (ohne Executions)
	@GetMapping
	...
	public ResponseEntity<TrainingSessionResponse> create(@Valid @RequestBody CreateSessionRequest req,
		...
		var saved = service.create(req.planId(), req.name(), req.scheduledDate());
	...
	public TrainingSessionResponse put(@PathVariable Long id, @Valid @RequestBody UpdateSessionRequest req) {
		var updated = service.update(id, req.planId(), req.name(), req.scheduledDate());
